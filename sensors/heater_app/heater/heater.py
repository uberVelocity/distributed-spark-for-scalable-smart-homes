...
class Heater:
    def __init__(self, watt_params, temp_params):
        """
        Constructor which specifies the heater sensor params.
    ...
    def compute_wattage(self, t):
        ...
        time_to_wattage = math.pow(2, t/4.8)
    ...
    def compute_heater(self, t):
        jitter = (random.random() * 2 * self.temp_vars['variance']) - self.temp_vars['variance']

        time_to_temperature = 0
        if t > self.temp_vars['t_fault']:
            time_to_temperature = - (t - self.temp_vars['t_fault']) / 18

        temperature = self.temp_vars['base'] + time_to_temperature + jitter
